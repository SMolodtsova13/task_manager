...
from task import Task, DATE_PATTERN_RU
...
def run_cli():
    ...
    while True:
        ...
        if choice == '1':
            ...
            due_date = Task.convert_data_from_str(input(f'Срок выполнения {DATE_PATTERN_RU}: '))
            priority = input('Приоритет (Низкий/Средний/Высокий): ')
            status_code = int(input('Статус задачи (1=Выполнено/0=Не выполнено): '))
            new_task = Task(
                id=None,
                title=title,
            ...
            new_due_date = input(f'Новый срок выполнения ({DATE_PATTERN_RU}) (оставьте пустым для сохранения текущего): ') or task.due_date
